...
def test_env_alias_prefers_ip_intel_names(monkeypatch) -> None:
    monkeypatch.setenv("NETWATCH_IP_INTEL_BATCH_SIZE", "777")
    monkeypatch.setenv("NETWATCH_LUPE_BATCH_SIZE", "123")
    assert lupe_enricher._env_int(200, "NETWATCH_IP_INTEL_BATCH_SIZE", "NETWATCH_LUPE_BATCH_SIZE") == 777
...
def test_resolve_provider_auto_prefers_maxmind(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("NETWATCH_IP_INTEL_PROVIDER", "auto")
    monkeypatch.setenv("NETWATCH_IP_INTEL_MAXMIND_CITY_DB_PATH", str(city))
    monkeypatch.setenv("NETWATCH_IP_INTEL_MAXMIND_ASN_DB_PATH", str(asn))
    monkeypatch.setenv("NETWATCH_IPINFO_TOKEN", "token-abc")
...
def test_resolve_provider_auto_falls_back_to_ipinfo(monkeypatch) -> None:
    monkeypatch.setenv("NETWATCH_IP_INTEL_PROVIDER", "auto")
    monkeypatch.delenv("NETWATCH_IP_INTEL_MAXMIND_CITY_DB_PATH", raising=False)
    monkeypatch.delenv("NETWATCH_IP_INTEL_MAXMIND_ASN_DB_PATH", raising=False)
    monkeypatch.setenv("NETWATCH_IPINFO_TOKEN", "token-abc")
    provider, reason = lupe_enricher._resolve_provider(lupe_enricher._provider_config())
