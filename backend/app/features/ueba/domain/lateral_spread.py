...
class LateralSpreadDetector:
    detector_id: str = "lateral_spread_v1"
    detector_version: int = 1
    metric_name: str = "lateral_distinct_dst_ips_per_window"
    ...
    def window_bounds(
        ...
        *,
        ...
        end = _floor_to_window(now, max(1, int(cfg.lateral_spread_window_minutes)))
        return end - timedelta(minutes=max(1, int(cfg.lateral_spread_window_minutes))), end

    def run(self, db: Session, *, cfg: DetectorRuntimeConfig, now: datetime) -> DetectorExecutionResult:
        window_minutes = max(1, int(cfg.lateral_spread_window_minutes))
        ...
        while (
            next_end <= latest_end
            and processed_windows < max(1, int(cfg.lateral_spread_max_windows_per_cycle))
        ):
            window_start = next_end - timedelta(minutes=window_minutes)
            rows, truncated = event_queries.fetch_lateral_spread_per_agent(
                db,
                window_started_at=window_start,
                window_ended_at=next_end,
                limit=max(1, int(cfg.lateral_spread_max_entities_per_cycle)),
            )
            ...
            keys_to_process: dict[str, tuple[str, int, int]] = {
                _baseline_key(self.detector_id, row.agent_id, "agent", row.agent_id, self.metric_name, "global"): (
                    row.agent_id,
                    row.distinct_dst_ips,
                    row.event_count,
                )
                for row in rows
            }
            ...
            for baseline_key, (agent_id, observed_ips, event_count) in keys_to_process.items():
        ...
        return DetectorExecutionResult(
            ...
            run_context_patch={
                ...
                "entity_limit": max(1, int(cfg.lateral_spread_max_entities_per_cycle)),
            },
        )
    ...
    def _updated_baseline_payload(
        ...
        *,
        ...
        stats = RunningStats.from_state(prior_state.get("running_stats")).update(max(0, int(observed_ips)))
        mature = stats.count >= max(1, int(cfg.lateral_spread_min_samples))
        status = "mature" if mature else "warmup"
        confidence = baseline_confidence(
            sample_count=stats.count,
            min_samples=max(1, int(cfg.lateral_spread_min_samples)),
            concentration=0.0,
        )
        ...
        return {
            ...
            "state": {
                "method": "welford_window_distinct_ips",
    ...
    def _evaluate_window(
        ...
        *,
        ...
        z_score = positive_z_score(
            observed_value=float(observed_ips),
            expected_value=expected,
            dispersion=dispersion,
        )
        if z_score < float(cfg.lateral_spread_min_z_score):
            return _NO_FINDING
        ...
        evidence = [
            {
                ...
                "matched_value": str(observed_ips)[:255],
                "summary": (
                    f"Agent {agent_id} reached {observed_ips} distinct internal IPs via lateral connections; "
                    f"baseline expected about {expected:.1f}"
                ),
                "raw_context": {
                    "agent_id": agent_id,
                    "observed_distinct_dst_ips": int(observed_ips),
                    "expected_distinct_dst_ips": float(expected),
        ...
        finding = _record_finding(
            ...
            summary=(
                f"Agent {agent_id} reached {observed_ips} distinct internal IPs via lateral connections "
                f"in one window; baseline expected about {expected:.1f}"
            ),
            reason_codes=["lateral_spread_above_baseline"],
            explanation={
                "method": "welford_window_distinct_ips",
                "window_minutes": max(1, int(cfg.lateral_spread_window_minutes)),
                "expected_distinct_dst_ips": expected,
                "observed_distinct_dst_ips": int(observed_ips),
            ...
            mitre_technique=technique_name("T1021"),
        )
        return finding
