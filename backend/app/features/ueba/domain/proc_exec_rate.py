...
class ProcExecRateDetector:
    detector_id: str = "proc_exec_rate_v1"
    detector_version: int = 1
    metric_name: str = "proc_exec_count_per_window"
    ...
    def window_bounds(
        ...
        *,
        ...
        end = _floor_to_window(now, max(1, int(cfg.proc_exec_rate_window_minutes)))
        return end - timedelta(minutes=max(1, int(cfg.proc_exec_rate_window_minutes))), end

    def run(self, db: Session, *, cfg: DetectorRuntimeConfig, now: datetime) -> DetectorExecutionResult:
        window_minutes = max(1, int(cfg.proc_exec_rate_window_minutes))
        ...
        while (
            next_end <= latest_end
            and processed_windows < max(1, int(cfg.proc_exec_rate_max_windows_per_cycle))
        ):
            window_start = next_end - timedelta(minutes=window_minutes)
            rows, truncated = event_queries.fetch_proc_exec_rate_per_agent(
                db,
                window_started_at=window_start,
                window_ended_at=next_end,
                limit=max(1, int(cfg.proc_exec_rate_max_entities_per_cycle)),
            )
            ...
            counters.scanned_events += sum(int(row.exec_count) for row in rows)
        ...
        return DetectorExecutionResult(
            ...
            run_context_patch={
                ...
                "entity_limit": max(1, int(cfg.proc_exec_rate_max_entities_per_cycle)),
            },
        )
    ...
    def _updated_baseline_payload(
        ...
        *,
        ...
        mature = stats.count >= max(1, int(cfg.proc_exec_rate_min_samples))
        status = "mature" if mature else "warmup"
        confidence = baseline_confidence(
            sample_count=stats.count,
            min_samples=max(1, int(cfg.proc_exec_rate_min_samples)),
            concentration=0.0,
        )
        ...
        return {
            ...
            "state": {
                "method": "welford_window_exec_count",
    ...
    def _evaluate_window(
        ...
        *,
        ...
        evidence = [
            {
                ...
                "entity_type": "agent",
                "entity_value": agent_id,
                "matched_field": "proc_exec_count",
                "matched_value": str(observed_count)[:255],
                "summary": (
                    f"Agent {agent_id} spawned {observed_count} processes in this window; "
                    f"baseline expected about {expected:.0f}"
                ),
                "raw_context": {
                    "agent_id": agent_id,
                    "observed_exec_count": int(observed_count),
                    "expected_exec_count": float(expected),
                },
            }
        ]
        finding = _record_finding(
            ...
            summary=(
                f"Agent {agent_id} spawned {observed_count} processes in one window; "
                f"baseline expected about {expected:.0f}"
            ),
            reason_codes=["proc_exec_rate_above_baseline"],
            explanation={
                "method": "welford_window_exec_count",
                "window_minutes": max(1, int(cfg.proc_exec_rate_window_minutes)),
                "expected_exec_count": expected,
                "observed_exec_count": int(observed_count),
