...
class SudoSessionHourDetector:
    detector_id: str = "sudo_session_hour_v1"
    detector_version: int = 1
    metric_name: str = "sudo_execution_hour"
    ...
    def window_bounds(
        ...
        *,
        ...
        return now - timedelta(hours=max(1, int(cfg.sudo_hour_lookback_hours))), now
    ...
    def run(self, db: Session, *, cfg: DetectorRuntimeConfig, now: datetime) -> DetectorExecutionResult:
        ...
        events = event_queries.fetch_sudo_cmd_events(
            db,
            after_id=after_id,
            since=since,
            limit=max(1, int(cfg.sudo_hour_max_events_per_cycle)),
        )
        ...
        return DetectorExecutionResult(
            ...
            run_context_patch={
                ...
                "lookback_hours": max(1, int(cfg.sudo_hour_lookback_hours)),
                "event_limit": max(1, int(cfg.sudo_hour_max_events_per_cycle)),
            },
        )
    ...
    def _updated_baseline_payload(
        ...
        *,
        ...
        mature = sample_count >= max(1, int(cfg.sudo_hour_min_samples))
        status = "mature" if mature else "warmup"
        confidence = baseline_confidence(
            sample_count=sample_count,
            min_samples=max(1, int(cfg.sudo_hour_min_samples)),
    ...
    def _evaluate_event(
        ...
        *,
        ...
        evidence = [
            {
                ...
                "summary": f"{event.username} ran sudo at {observed_hour:02d}:00 UTC",
                "raw_context": {
                    "agent_id": event.agent_id,
                    "event_type": "sudo_cmd",
        ...
        finding = _record_finding(
            ...
            summary=(
                f"Sudo execution by {event.username} at {observed_hour:02d}:00 UTC "
                f"deviated from the usual {snapshot.mode_hour:02d}:00 UTC hour"
            ),
            reason_codes=["rare_sudo_hour", "sudo_hour_far_from_baseline"],
            explanation={
                "method": "hour_histogram",
