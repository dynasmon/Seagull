...
class OutboundBytesBurstDetector:
    detector_id: str = "outbound_bytes_burst_v1"
    detector_version: int = 1
    metric_name: str = "outbound_bytes_per_window"
    ...
    def window_bounds(
        ...
        *,
        ...
        end = _floor_to_window(now, max(1, int(cfg.outbound_bytes_window_minutes)))
        return end - timedelta(minutes=max(1, int(cfg.outbound_bytes_window_minutes))), end

    def run(self, db: Session, *, cfg: DetectorRuntimeConfig, now: datetime) -> DetectorExecutionResult:
        window_minutes = max(1, int(cfg.outbound_bytes_window_minutes))
        ...
        while (
            next_end <= latest_end
            and processed_windows < max(1, int(cfg.outbound_bytes_max_windows_per_cycle))
        ):
            window_start = next_end - timedelta(minutes=window_minutes)
            rows, truncated = event_queries.fetch_outbound_bytes_per_agent(
                db,
                window_started_at=window_start,
                window_ended_at=next_end,
                limit=max(1, int(cfg.outbound_bytes_max_entities_per_cycle)),
            )
            ...
            for baseline_key, (agent_id, observed_bytes, event_count) in keys_to_process.items():
        ...
        return DetectorExecutionResult(
            ...
            run_context_patch={
                ...
                "entity_limit": max(1, int(cfg.outbound_bytes_max_entities_per_cycle)),
            },
        )
    ...
    def _updated_baseline_payload(
        ...
        *,
        ...
        stats = RunningStats.from_state(prior_state.get("running_stats")).update(max(0, int(observed_bytes)))
        mature = stats.count >= max(1, int(cfg.outbound_bytes_min_samples))
        status = "mature" if mature else "warmup"
        confidence = baseline_confidence(
            sample_count=stats.count,
            min_samples=max(1, int(cfg.outbound_bytes_min_samples)),
            concentration=0.0,
        )
    ...
    def _evaluate_window(
        ...
        *,
        ...
        z_score = positive_z_score(
            observed_value=float(observed_bytes),
            expected_value=expected,
            dispersion=dispersion,
        )
        if z_score < float(cfg.outbound_bytes_min_z_score):
            return _NO_FINDING
        ...
        evidence = [
            {
                ...
                "entity_value": agent_id,
                "matched_field": "bytes",
                "matched_value": str(observed_bytes)[:255],
                "summary": (
                    f"Agent {agent_id} sent {observed_bytes} bytes outbound in this window; "
        ...
        finding = _record_finding(
            ...
            summary=(
                f"Agent {agent_id} sent {observed_bytes} bytes outbound in one window; "
                f"baseline expected about {expected:.0f}"
            ),
            reason_codes=["outbound_bytes_above_baseline"],
            explanation={
                "method": "welford_window_bytes",
                "window_minutes": max(1, int(cfg.outbound_bytes_window_minutes)),
            ...
            mitre_technique=technique_name("T1048"),
        )
        return finding
