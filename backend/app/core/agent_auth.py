...
def generate_agent_token(agent_id: str) -> Tuple[str, str, str]:
    """
    Legacy token format: <agent_id>.<secret>
...
def _get_current_agent_bearer(request: Request) -> AgentPrincipal:
    ...
    try:
        ...
        return AgentPrincipal(id=agent.id, agent_id=agent.agent_id, auth_method="bearer")
    finally:
        db.close()


def get_current_agent(request: Request) -> AgentPrincipal:
    mode = (settings.NETWATCH_AGENT_AUTH_MODE or "mtls").strip().lower()
    ...
    if mode == "mixed":
        mtls_headers_present = _extract_mtls_headers(request) is not None
        if mtls_headers_present:
            principal = _get_current_agent_mtls(request, strict=True)
            if principal is None:
                raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Missing mTLS identity")
            return principal
        return _get_current_agent_bearer(request)

    # Legacy emergency mode.
    return _get_current_agent_bearer(request)
