...
def recent_feed_health(*, agent_id: str | None = None) -> Dict[str, Any]:
    ...
    try:
        ...
        last_ts_iso = None
    try:
        ts_s = int(datetime.now(timezone.utc).timestamp()) - 1
        events_last_second = int(r.get(_feed_rate_key(ts_s)) or 0)
        dropped_last_second = int(r.get(_feed_drop_key(ts_s)) or 0)
    except Exception:
        events_last_second = 0
        dropped_last_second = 0
