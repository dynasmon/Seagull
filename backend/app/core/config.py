...
class Settings:
    ...
    NETWATCH_RULES_ENABLED_PACKS: list[str] = _env_csv(
        ...
        "core,network,host" if NETWATCH_ENV in {"prod", "production"} else "core,network,host,lab",
    )
    ...
    NETWATCH_HEUR_MAX_ROWS: int = _env_int("NETWATCH_HEUR_MAX_ROWS", 50000)
    NETWATCH_HEUR_BEACON_WINDOW_SECONDS: int = _env_int("NETWATCH_HEUR_BEACON_WINDOW_SECONDS", 3600)
    NETWATCH_HEUR_BEACON_MIN_EVENTS: int = _env_int("NETWATCH_HEUR_BEACON_MIN_EVENTS", 7)
    NETWATCH_HEUR_BEACON_MIN_INTERVAL_SECONDS: float = _env_float("NETWATCH_HEUR_BEACON_MIN_INTERVAL_SECONDS", 8.0)
    NETWATCH_HEUR_BEACON_MAX_INTERVAL_SECONDS: float = _env_float("NETWATCH_HEUR_BEACON_MAX_INTERVAL_SECONDS", 900.0)
    NETWATCH_HEUR_BEACON_MAX_JITTER: float = _env_float("NETWATCH_HEUR_BEACON_MAX_JITTER", 0.22)
    NETWATCH_HEUR_BEACON_COOLDOWN_SECONDS: int = _env_int("NETWATCH_HEUR_BEACON_COOLDOWN_SECONDS", 900)

    NETWATCH_HEUR_EXFIL_BASELINE_SECONDS: int = _env_int("NETWATCH_HEUR_EXFIL_BASELINE_SECONDS", 86400)
    NETWATCH_HEUR_EXFIL_WINDOW_SECONDS: int = _env_int("NETWATCH_HEUR_EXFIL_WINDOW_SECONDS", 600)
    NETWATCH_HEUR_EXFIL_MIN_EVENTS: int = _env_int("NETWATCH_HEUR_EXFIL_MIN_EVENTS", 8)
    NETWATCH_HEUR_EXFIL_MIN_BYTES: int = _env_int("NETWATCH_HEUR_EXFIL_MIN_BYTES", 8 * 1024 * 1024)
    NETWATCH_HEUR_EXFIL_SPIKE_FACTOR: float = _env_float("NETWATCH_HEUR_EXFIL_SPIKE_FACTOR", 3.0)
    NETWATCH_HEUR_EXFIL_RARE_BASELINE_EVENTS: int = _env_int("NETWATCH_HEUR_EXFIL_RARE_BASELINE_EVENTS", 5)
    NETWATCH_HEUR_EXFIL_COOLDOWN_SECONDS: int = _env_int("NETWATCH_HEUR_EXFIL_COOLDOWN_SECONDS", 1200)

    NETWATCH_HEUR_EGRESS_BASELINE_SECONDS: int = _env_int("NETWATCH_HEUR_EGRESS_BASELINE_SECONDS", 21600)
    NETWATCH_HEUR_EGRESS_WINDOW_SECONDS: int = _env_int("NETWATCH_HEUR_EGRESS_WINDOW_SECONDS", 900)
    NETWATCH_HEUR_EGRESS_MIN_EVENTS: int = _env_int("NETWATCH_HEUR_EGRESS_MIN_EVENTS", 5)
    NETWATCH_HEUR_EGRESS_MIN_BYTES: int = _env_int("NETWATCH_HEUR_EGRESS_MIN_BYTES", 2 * 1024 * 1024)
    NETWATCH_HEUR_EGRESS_SPIKE_FACTOR: float = _env_float("NETWATCH_HEUR_EGRESS_SPIKE_FACTOR", 2.5)
    NETWATCH_HEUR_EGRESS_RARE_BASELINE_EVENTS: int = _env_int("NETWATCH_HEUR_EGRESS_RARE_BASELINE_EVENTS", 3)
    NETWATCH_HEUR_EGRESS_CORRELATION_SECONDS: int = _env_int("NETWATCH_HEUR_EGRESS_CORRELATION_SECONDS", 900)
    NETWATCH_HEUR_EGRESS_COOLDOWN_SECONDS: int = _env_int("NETWATCH_HEUR_EGRESS_COOLDOWN_SECONDS", 1200)
