...
Last updated: 2026-05-30 (Phase 5 area 5: Exposure & Vulnerabilities).

---

## TL;DR — where we stopped

Phases **1, 2, 3, 4 are DONE**. **Phase 5 areas 1 (Alerts), 2 (Events), 3 (Entities/Inventory), 4 (Network/Topology), 5 (Exposure & Vulnerabilities) are DONE**, plus the shared **`DataView` primitives** migrated to EUI (cross-cutting, ~21 pages). Next Phase 5 area is **Investigations & Cases** (`src/features/investigations/`, `src/features/attack_chain/`). All work passed the verification baseline at every batch. Exposure was already mostly on adapters (only filter/graph checkboxes + page tabs left); Vulnerabilities converted its **three raw `<table>` surfaces to the shared `Table`**, plus scans filters/buttons and two clickable card-lists→`EuiPanel`. Open browser passes needed (no browser in this env): Alerts, Events (4 lenses), Entities/Inventory, Network/Topology, Exposure, Vulnerabilities, and especially the cross-cutting `DataView` SWEEP across every data page. Two cross-cutting items still deferred (each needs its own focused pass): **`DraftNumberInput`→`EuiFieldNumber`** (now mainly the Vulnerabilities page-size/min inputs + the Topology Confidence input) and the **`ResponseActionDrawer`** (750-line Response/Automation surface, 15 varied buttons).
...
6. **Investigations & Cases** (`src/features/investigations/`, `src/features/attack_chain/`) — **START HERE.**
