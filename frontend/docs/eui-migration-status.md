...
1. The deferred **`DraftNumberInput`→`EuiFieldNumber`** cross-cutting batch — **8 call sites across 6 files** (all hosts now migrated): `events/components/EventsFilters.tsx` ×1, `inventory/components/InventoryScopePanel.tsx` ×1, `agents/components/AgentEventsWorkbench.tsx` ×2, `vulnerabilities/page.tsx` ×2, `vulnerabilities/scans.tsx` ×1, `network_topology/components/filters/TopologyFilterRail.tsx` ×1. They pass divergent styling (clean classes / `.ui-input` / fully custom border-bg-sizing), so migrate the shared `DraftNumberInput` internals to `EuiFieldNumber` while keeping its anti-jump draft/commit API (`value`/`onCommit`/`min`/`max`/`fallback`), then reconcile each consumer's `className` in the same pass.
