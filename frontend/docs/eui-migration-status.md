...
Last updated: 2026-05-30 (Phase 5 area 3: Entities / Inventory).

---

## TL;DR — where we stopped

Phases **1, 2, 3, 4 are DONE**. **Phase 5 areas 1 (Alerts), 2 (Events), 3 (Entities/Inventory) are DONE**, plus the shared **`DataView` primitives** migrated to EUI (cross-cutting, ~21 pages). Next Phase 5 area is **Network / Topology** (`src/features/network_topology/`). All work passed the verification baseline at every batch. Open browser passes needed (no browser in this env): Alerts, Events (4 lenses), Entities/Inventory, and especially the cross-cutting `DataView` SWEEP across every data page. Two cross-cutting items still deferred (each needs its own focused pass): **`DraftNumberInput`→`EuiFieldNumber`** (8 sites/6 files, divergent styling) and the **`ResponseActionDrawer`** (750-line Response/Automation surface, 15 varied buttons).
...
`src/shared/components/IconButton.tsx`, `QueryState.tsx`, `FormField.tsx`, `ThemeToggle.tsx`; `src/features/overview/components/StatLinkTile.tsx`; the whole `src/features/_eui_lab/` + its `/eui-lab` route in `src/app/routes.tsx`.
...
4. **Network / Topology** (`src/features/network_topology/`) — **START HERE.** keep sigma/graphology/@xyflow; tests fail at collection (no jsdom) — that's baseline. `TopologyFilterRail` is a `DraftNumberInput` host (custom-styled input — see the deferred `DraftNumberInput` note).
5. **Exposure & Vulnerabilities** (`src/features/exposure/`, `src/features/vulnerabilities/`).
