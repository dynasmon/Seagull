...
Last updated: 2026-05-30 (Phase 5 area 4: Network / Topology).

---

## TL;DR — where we stopped

Phases **1, 2, 3, 4 are DONE**. **Phase 5 areas 1 (Alerts), 2 (Events), 3 (Entities/Inventory), 4 (Network/Topology) are DONE**, plus the shared **`DataView` primitives** migrated to EUI (cross-cutting, ~21 pages). Next Phase 5 area is **Exposure & Vulnerabilities** (`src/features/exposure/`, `src/features/vulnerabilities/`). All work passed the verification baseline at every batch. Network/Topology was already largely on the shared adapters — this pass cleared the residual standard-UI raw bits, deleted dead `TopologyTopBar`, and **intentionally kept the `@xyflow` graph-canvas chrome** (floating controls/legend/context-menu/status-strip/tooltip — the canonical "keep sigma/graphology/@xyflow"). Open browser passes needed (no browser in this env): Alerts, Events (4 lenses), Entities/Inventory, Network/Topology, and especially the cross-cutting `DataView` SWEEP across every data page. Two cross-cutting items still deferred (each needs its own focused pass): **`DraftNumberInput`→`EuiFieldNumber`** (8 sites/6 files, divergent styling — still includes the Topology filter rail Confidence input) and the **`ResponseActionDrawer`** (750-line Response/Automation surface, 15 varied buttons).
...
5. **Exposure & Vulnerabilities** (`src/features/exposure/`, `src/features/vulnerabilities/`) — **START HERE.** Multiple `DraftNumberInput` hosts live here (vulns ×3) — see the deferred cross-cutting note before touching them.
6. **Investigations & Cases** (`src/features/investigations/`, `src/features/attack_chain/`).
7. **Governance & Platform** (`src/features/audit/`, `src/features/internal/`).
