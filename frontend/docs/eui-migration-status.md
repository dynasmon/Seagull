...
Last updated: 2026-05-31 (Kibana row-density pass **COMPLETE**; UEBA/Anomaly Detection "Area 9" **COMPLETE**; ResponseActionDrawer, DraftNumberInput, all 8 original feature areas done). **Browser QA via headless Chrome/CDP, dark+light verified for UEBA.**

---

## ▶ NEXT SESSION — resume here (updated 2026-05-31)

**UEBA/Anomaly Detection "Area 9" is COMPLETE.** The last raw feature-area gap is closed: UEBA findings + detector runs now render through the shared EUI `Table` (`EuiBasicTable`), UEBA filters/buttons use shared adapters, and the finding drawer verdict controls use shared EUI-backed controls.

**What shipped this session (NOT yet committed — the human commits):**
- **Single-line cell collapse** in every remaining multi-line table: Vulnerabilities **findings** (`page.tsx`) + **scans** (`scans.tsx`, `ActiveScanPanel.tsx`), **DDoS** (`DdosEventsTable.tsx`), **Investigations** workspaces (`investigations/page.tsx`), **Attack-chain** cases (`attack_chain/page.tsx`), **Audit** events (`AuditEventsTable.tsx`), **Correlations** incidents (`CorrelationIncidentTable.tsx`) + rules (`views/rules.tsx`), **Exposure** assets (`ExposureAssetTable.tsx`) + findings (`ExposureFindingList.tsx`), **UEBA** findings (`ueba/views/findings.tsx`). Pattern: `flex items-center gap-1.5 min-w-0`, primary value `truncate`, secondary `shrink-0`, full content in the cell `title`.
- **Shared `StatusPill` hardened** (`shared/components/StatusPill.tsx`) → `cx("shrink-0 whitespace-nowrap", className)`. **Root-cause fix:** an `EuiHealth` (StatusPill) is the only shrinkable item when placed in a single-line flex with `EuiBadge`s (badges have flex-shrink:0), so flex squeezed it and its label wrapped **per-character** → 80px-tall rows (caught on Correlations incidents: 89px). `shrink-0` stops the squeeze; `whitespace-nowrap` stops the wrap. This protects every single-line row that includes a StatusPill (Investigations, Attack-chain, Correlations).
- **`scrollX` added to the 3 WIDE tables** (≥9 cols) so they scroll within their panel instead of cramming (the doc's "wide tables scroll, not cram" — same as Events network/SSH): **Exposure assets** (12 cols, was 153px→41px), **Exposure findings** (10 cols), **Correlations rules** (9 cols, was 73px→41px). `.seagullTable-scrollX .euiTable { inline-size: max-content }` gives each column its full width so bare-string cells (agent ids, timestamps) stop wrapping.
- **Two opt-in `nowrap` props** added to shared/feature chip components so table cells stay single-line while their drawer/other uses keep wrapping: `ScanStats` (`ActiveScanPanel.tsx`) and `ExposureReasonCodes`. Local `chip`/`stageHitPreview` helpers in `CorrelationIncidentTable.tsx` switched `flex-wrap`→`flex-nowrap` + `shrink-0`.
- **Events stream (`EventsTable`) intentionally left comfortable** (the documented exception — its `compact` toggle also adds the details column). **Inventory** main tables + **Internal** (health/debug/agents) tables + **Audit logins** were **already single-line** — no change needed.
- **UEBA Area 9:** `ueba/views/findings.tsx` raw table → shared `Table` with sort wiring, selected-row highlight, sticky header, explicit widths + `scrollX`; findings filters (`detector`, `severity`, `status`) → `SelectInput`; Refresh/Retry → shared `Button`. `ueba/views/detectors.tsx` detector-runs raw table → shared `Table` with `expandedRowMap` run details; runs filter → `SelectInput`; Retry → `Button`. `ueba/components/FindingDrawer.tsx` verdict buttons/Cancel/Confirm → shared `Button`; override checkbox → `CheckboxField`; verdict error → `InlineAlert`. Shared `Table` gained optional `expandedRowMap` support for EUI expanded rows.

**UEBA grep check:** no raw `<table>`, `<select>`, `ui-select`, `ui-input`, `type="checkbox"`, `ui-btn`, or underline retry controls remain under `src/features/ueba`. The only remaining raw `<button>` is the detector-card "Open findings" metric tile, kept as a domain clickable tile rather than a standard control.

**Verification (this session):** build exit 0 · lint 0-err/12-warn · tests at baseline (16 failed files / 3 failed assertions / 81 passed) · eager entry `index-*.js` **114.58 KB gz**. In-browser (dark+light): Vuln scans 41px, Correlations incidents 41px (29px short rows), Correlations rules 41px, Investigations 41px, Audit 41px, Exposure assets 41px, UEBA findings 29px, UEBA detector runs 29px — all no cell overflow and no page horizontal overflow. UEBA finding drawer opens from row click; detector run rows expand with preserved details. Empty in this dataset (pattern-only, **human visual check owed with real data**): Vulnerabilities **findings**, Attack-chain cases, DDoS events.
...
- `views/detectors.tsx` — raw recent-runs `<table>` → shared **`Table`** with `scrollX`, selected expanded row highlight, and `expandedRowMap` run details (window, baselines, alerts, error/context) via the shared adapter's new EUI expanded-row pass-through. Runs detector filter → **`SelectInput`**; Retry → **`Button`**.
