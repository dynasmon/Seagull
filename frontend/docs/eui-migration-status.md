...
Last updated: 2026-05-30 (Phase 5 area 6: Investigations & Cases).

---

## TL;DR — where we stopped

Phases **1, 2, 3, 4 are DONE**. **Phase 5 areas 1 (Alerts), 2 (Events), 3 (Entities/Inventory), 4 (Network/Topology), 5 (Exposure & Vulnerabilities), 6 (Investigations & Cases) are DONE**, plus the shared **`DataView` primitives** migrated to EUI (cross-cutting, ~21 pages). Next Phase 5 area is **Governance & Platform** (`src/features/audit/`, `src/features/internal/`). All work passed the verification baseline at every batch. Investigations & Attack Chain were already on the shared adapters — only one raw `<table>` per page remained, both converted to the shared `Table`. Open browser passes needed (no browser in this env): Alerts, Events (4 lenses), Entities/Inventory, Network/Topology, Exposure, Vulnerabilities, Investigations, Attack Chain, and especially the cross-cutting `DataView` SWEEP across every data page. Two cross-cutting items still deferred (each needs its own focused pass): **`DraftNumberInput`→`EuiFieldNumber`** (now mainly the Vulnerabilities page-size/min inputs + the Topology Confidence input) and the **`ResponseActionDrawer`** (750-line Response/Automation surface, 15 varied buttons).
...
7. **Governance & Platform** (`src/features/audit/`, `src/features/internal/`) — **START HERE.**
